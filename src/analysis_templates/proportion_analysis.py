...
class ProportionAnalysis(AnalysisTemplate):
    spec=TemplateSpec(analysis_type="proportion_analysis",display_name="占比分析",
        REQUIRED_SCHEMA={"dimension_type":"category","metric_type":"numeric","min_dimension":1,"min_metric":1},
        MIN_ROWS=1,MIN_DISTINCT_VALUES=1,FALLBACK=None,
        OUTPUT_CHARTS=["pie"],OUTPUT_TABLES=["summary"],OUTPUT_KPIS=["max_pct","cat_count"])
    def execute(self,df,dimension,metric,algorithm=None):
        ...
        total=grouped.sum(); max_pct=grouped.max()/total*100 if total else 0
        ...
        charts=[ChartData(slot="proportion",chart_type="pie",title=f"{dimension}{metric}占比",x=dimension,y=metric)]
        return AnalysisPackage(id="",analysis_type="proportion_analysis",business_question="",algorithm=algorithm,
