...
/** 触发 CSV 下载（原生 GET 方式，规避 blob.click 在部分浏览器不触发的坑） */
export async function downloadCSV(sessionId: string, original = false) {
  const params = new URLSearchParams({ session_id: sessionId, export_original: String(original) });
  const url = `${API_BASE}/data/download?${params.toString()}`;

  // 先 fetch 探测：成功才触发下载；失败（如 session 数据已丢失 → 404）则把后端错误信息显示出来
  ...
  if (!resp.ok) {
    ...
    try {
      ...
      msg = j.detail || msg;
    } catch { /* 非 JSON 错误体时保持默认提示 */ }
    throw new Error(msg);
  }

  // 成功：用原生 <a> 下载（比 blob.click 兼容性更好，几乎不被浏览器/扩展拦截）
