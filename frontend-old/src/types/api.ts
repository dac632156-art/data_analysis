...
export interface EChartItem {
  title: string;
  option: Record<string, unknown>;
  /** 图表类型：''=普通图表, 'table'=同环比表格, 'analysis_table'=数据表, 'seg_profile_overview'=群画像总览表 */
  chart_type?: string;
  /** 同环比表格数据 */
  table_data?: Record<string, unknown>;
  /** 高级表格/图表元数据（前端按 kind 分支渲染，如群画像总览表的区块与颜色编码） */
...
export interface PackageTableData {
  title: string;
  table_type: 'summary' | 'ranking' | 'cross' | 'growth' | 'correlation' | 'detail' | 'exception' | 'profile_overview' | 'ar_rules' | 'ar_rules_category' | 'ar_rules_segment';
  columns: string[];
  rows: unknown[][];
  /** 高级表格元数据（前端按 kind 分支渲染，如群画像总览表的区块与颜色编码） */
  chart_config?: Record<string, unknown>;
  /** 与图表一致的定位标识，如 "rfm_segment_summary_table" */
  slot?: string;
}
