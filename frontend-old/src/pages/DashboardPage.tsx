...
export default function DashboardPage() {
  ...
  const [exportPkgList, setExportPkgList] = useState<Array<Record<string, unknown>>>([]);
      ...
      if (res && res.packages && res.packages.length > 0) {
        ...
        for (const pkg of res.packages) {
          ...
          for (const c of pkgCharts) {
            // 空 option（{}）视为无内容，不进入渲染列表，避免空白卡片占位
            if (c && c.option && Object.keys(c.option).length > 0) {
              if (c.chart_type === 'hbar_family') {
                // 图族（流失归因 F）：option = {维度名: 子图option}，
                // 看板摄入时摊平成 N 个普通横向 bar，EGridLayout 零改动即可渲染
                for (const [dim, subOption] of Object.entries(c.option as Record<string, Record<string, unknown>>)) {
                  if (subOption && Object.keys(subOption).length > 0) {
                    allCharts.push({
                      title: `${c.title || '流失归因'} · ${dim}`,
                      option: subOption,
                      x: c.x || '',
                      y: c.y || '',
          ...
          for (const t of pkgTables) {
            if (t && t.rows && t.columns) {
              // RenderedTable 的 rows 是 List[List[RenderedCell]]
              const isOverview = !!(t.chart_config && (t.chart_config as any).kind === 'seg_profile_overview');
              if (isOverview) {
                // 群画像总览表：保留 cell 元数据（value/rank/direction/cell_type）+ 区块结构
                const tableRows = t.rows.map((row: any[]) => row.map((cell: any) =>
                  cell && typeof cell === 'object' && 'value' in cell
                    ? { value: cell.value, rank: cell.rank ?? 0, direction: cell.direction ?? 'neutral', cell_type: cell.cell_type ?? 'text' }
                    : { value: cell, rank: 0, direction: 'neutral', cell_type: 'text' }
                ));
                allCharts.push({
                  title: t.title || '群画像总览表',
                  option: {},
                  x: '',
                  y: '',
                  analysis_type: pkg.analysis_type || '',
                  chart_type: 'seg_profile_overview',
  ...
  const handleDownloadPackages = async () => {
    ...
    try {
      ...
      // 默认全选，打开勾选面板
      const allIds = new Set(packages.map((p: any) => p.id as string));
      setExportPkgList(packages);
      setExportSelected(allIds);
      setShowExportPicker(true);
    } catch (e) {
      alert('加载分析包列表失败：' + (e instanceof Error ? e.message : '未知错误'));
    }
  };

  // 按勾选结果过滤并导出（charts.option 完整保留）
  const confirmExportSelected = () => {
    const selectedPkgs = exportPkgList.filter((p) => exportSelected.has(p.id as string));
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {showExportPicker && (
        ...
        >
          <div
            className="w-full max-w-[480px] max-h-[80vh] flex flex-col rounded-xl bg-[#0F172A] border border-[#8B5CF6]/30 shadow-[0_0_40px_rgba(139,92,246,0.25)]"
            onClick={(e) => e.stopPropagation()}
          >
            {/* 标题区 */}
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#8B5CF6]/20">
              <h3 className="text-base font-semibold text-[#38BDF8]">选择要导出的分析包</h3>
              <span className="text-xs text-slate-400">
                已选 {exportSelected.size} / {exportPkgList.length}
              </span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2">
              {exportPkgList.map((p) => {
                ...
                const checked = exportSelected.has(id);
                const type = (p.analysis_type as string) || '未命名模型';
                const question = (p.business_question as string) || '';
                const chartCount = Array.isArray(p.charts) ? (p.charts as unknown[]).length : 0;
                return (
                  <button
                    key={id}
                    type="button"
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg border text-left transition-colors ${
                      checked
                        ? 'border-[#22D3EE]/50 bg-[#22D3EE]/[0.06]'
                        : 'border-slate-700/40 bg-[#1E293B]/30 hover:bg-[#A78BFA]/[0.08]'
                    }`}
                  >
                    <span
                      className={`flex-shrink-0 w-4 h-4 rounded border flex items-center justify-center text-[10px] ${
                        checked ? 'bg-[#22D3EE] border-[#22D3EE] text-[#020617]' : 'border-slate-500'
                      }`}
                    >
                      {checked ? '✓' : ''}
                    </span>
                    <span className="flex-1 min-w-0">
                      <span className="inline-block px-2 py-0.5 rounded-full text-[10px] bg-[#A78BFA]/20 text-[#A78BFA] border border-[#A78BFA]/20">
                        {type}
                      </span>
                      {question && (
                        <span className="block mt-1 text-[13px] text-slate-100 truncate">{question}</span>
                      )}
                    </span>
                    <span className="flex-shrink-0 text-xs text-[#94A3B8]">{chartCount} 张图</span>
                  </button>
                );
              })}
            </div>
            ...
            {/* 操作栏 */}
            <div className="flex items-center justify-between gap-3 px-5 py-4 border-t border-[#8B5CF6]/20">
              <div className="flex gap-3 text-xs">
                <button
                  type="button"
                  onClick={() => setExportSelected(new Set(exportPkgList.map((p) => p.id as string)))}
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => setShowExportPicker(false)}
                  className="px-4 py-1.5 text-xs rounded-lg border border-slate-600 text-slate-300 hover:bg-slate-700/30 transition-colors"
                >
                  取消
                </button>
                <button
                  ...
                  className="px-4 py-1.5 text-xs rounded-lg bg-[#8B5CF6] text-white hover:bg-[#7C4DF0] hover:shadow-[0_0_16px_rgba(139,92,246,0.45)] transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  导出选中
                </button>
              </div>
