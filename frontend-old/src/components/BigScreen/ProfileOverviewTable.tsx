export interface OverviewCell {
  ...
  highlight?: boolean;  // 新增：该行（该类用户）人均净毛利最高的地区高亮
  count?: number;       // 新增：该组合客户数，用于副标防误导
}
...
// dirs 的 key = 后端 feature_cols（展示名，含单位）；value = 该特征 direction(good/bad/equal)。
// 极性已由后端按 feature_polarity 翻转（如静默天数越高越差=bad），故此处直接吃 direction。
// 注：spec 里"母婴偏好/夜猫子/高运费"等需类目/时段语义，当前特征仅有通用统计量，
// 规则按现有 direction 做近似映射（如跨类目广度高→多类目交叉销售）。
