...
import ProfileOverviewTable from './ProfileOverviewTable';
      ...
      <div className={`grid grid-cols-3 gap-4 p-6 ${showTable ? '' : 'flex-1'} overflow-auto`}>
        {echarts.length > 0 ? (
          echarts.map((chart, i) => {
            ...
            const isOverview = chart.chart_type === 'seg_profile_overview' && chart.table_data && chart.chart_config;
            const gridSpan = isGL || isOverview ? { gridColumn: 'span 3' } : isAnalysisTable ? { gridColumn: 'span 3' } : {};
              ...
              <div key={i} style={gridSpan}>
                <BorderBox1 color={['#7DD3FC', '#38BDF8']} style={{ padding: '6px' }}>
                  {isGL ? (
                    ...
                    <ProfileOverviewTable chart={chart as any} hideTitle={hideChartTitle} />
                  ) : isAnalysisTable ? (
