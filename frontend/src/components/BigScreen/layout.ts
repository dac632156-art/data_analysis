/**
 * layout.ts —— 从 SmartDashboard.tsx 抽出的纯布局函数模块。
 *
 * 这些函数无 React / DOM 依赖，纯数据变换，供 SmartDashboard 渲染与
 * 导出（exportComponentHTML）复用，避免导出函数 import 整个 SmartDashboard
 * 组件树（含 EtherealCharts 重度依赖）造成循环引用与重依赖。
 *
 * 数据流（真实链路）：
 *   saved_packages → extractChartsFromSavedPackages → { charts, items }
 *   → buildSemanticLayout(charts, items, mode) → ComputeLayoutResult
 */
...
export type { ChartLike, ChartBucket, RowSpec };
