    ...
    >
      ...
      <div className="relative flex items-center justify-between px-8 py-4 border-b"
        ...
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-[#0f172a] tracking-widest"
            style={{}}>
            {title}
          </h1>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {topKpis.length > 0 && (
          <section className="rounded-2xl p-5"
            style={{ background: 'rgba(255,255,255,0.35)', border: '1px solid rgba(255,255,255,0.45)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)' }}>
        ...
        <div className="grid gap-6" style={{ gridTemplateColumns: '1fr' }}>
          ...
          <section className="rounded-2xl p-5"
            ...
            <div className="space-y-6">
              {mainTrend && <ChartBlock card={mainTrend} />}
              {subTrends.length > 0 && (
                <div className="grid gap-6" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))' }}>
                  {subTrends.map((t) => <ChartBlock key={t.id} card={t} />)}
                </div>
              )}
            </div>
          </section>
...
function KpiMiniCard({ card }: { card: CardItem }) {
  ...
  return (
    <div className="rounded-xl p-4"
      style={{ background: 'rgba(255,255,255,0.40)', border: '1px solid rgba(255,255,255,0.45)', backdropFilter: 'blur(10px)', WebkitBackdropFilter: 'blur(10px)' }}>
...
function ChartBlock({ card }: { card: CardItem }) {
  const option = (card.data?.option as EChartsOption | undefined) || (card.data as EChartsOption | undefined);
  if (!option) return null;
  return (
    <div className="rounded-xl p-4"
      style={{ background: 'rgba(255,255,255,0.35)', border: '1px solid rgba(255,255,255,0.45)', backdropFilter: 'blur(10px)', WebkitBackdropFilter: 'blur(10px)' }}>
      <h3 className="text-sm font-semibold text-cyan-400 mb-3">{card.title}</h3>
      <EtherealChart slot={card.slot} chartType={card.chart_type} chartNode={option} title={card.title} />
    </div>
  );
}
          ...
          <tr>
            {columns.map((col, i) => (
              <th key={i} className="py-2 px-3 text-left font-medium text-slate-400"
                style={{ borderBottom: '1px solid rgba(255,0,0,0)' }}>
                {String(col)}
              </th>
            ))}
          </tr>
...
function WarningBlock({ card }: { card: CardItem }) {
  ...
  return (
    <div className="rounded-xl p-3 flex items-start gap-2"
      ...
      <p className="text-xs text-slate-700 leading-relaxed">{String(text)}</p>
    </div>
  );
}
